...
class Content {
    ...
    // Messages coming from MESE injected library.
            ...
            try {
                if (typeof d === 'string') {
                    ...
                    if(Object.prototype.toString.call(d) === '[object Object]' && "source" in d){
                        if(d.source == "mese-dapp") {
                            let eventId: string = d.body.id;
                            ctx.events[eventId] = ev;
                        }
                        if(d.source == "mese-dapp" || d.source == "router") {
    ...
    // They are sent to the MESE injected library.
