/**
 * dapp e2e tests for the MESE Multisig functionality
 *
 * @group dapp/multisig
 */
...
const { CreateWallet, ConnectMESE, ImportAccount } = require('./common/tests');
  ...
  test('Fail on Sign with no Valid Addresses', async () => {
    ...
    await expect(
      dappPage.evaluate((transaction) => {
        return Promise.resolve(MESE.signMultisig(transaction))
          .then((data) => {
            return data;
          })
          .catch((error) => {
